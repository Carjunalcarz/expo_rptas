...
const AssessmentSearch: React.FC<AssessmentSearchProps> = ({
  assessments,
  onFilteredResults,
  ...
  const filterAssessments = (assessmentList: any[], currentFilters: SearchFilters) => {
    return assessmentList.filter(assessment => {
      // Text search - search in owner name, address, and assessment ID
      if (currentFilters.searchText) {
        const searchLower = currentFilters.searchText.toLowerCase();
        const owner = assessment.data?.owner_details?.owner?.toLowerCase() || '';
        const localId = String(assessment.local_id || '').toLowerCase();
        const street = assessment.data?.building_location?.street?.toLowerCase() || '';
        const barangay = assessment.data?.building_location?.barangay?.toLowerCase() || '';
        const municipality = assessment.data?.building_location?.municipality?.toLowerCase() || '';
        const province = assessment.data?.building_location?.province?.toLowerCase() || '';
        
        const matchesText = owner.includes(searchLower) ||
                           localId.includes(searchLower) ||
      ...
      if (currentFilters.status !== 'all') {
        const isSynced = !!assessment.synced;
        if (currentFilters.status === 'synced' && !isSynced) return false;
        if (currentFilters.status === 'pending' && isSynced) return false;
      }

      // Date range filter
      if (currentFilters.dateRange !== 'all' && assessment.created_at) {
      ...
      if (currentFilters.valueRange !== 'all') {
        const marketValue = assessment.data?.property_assessment?.market_value || 0;
      ...
      {showFilters && (
        <View className="mt-3 bg-white border border-gray-200 rounded-xl p-4">
          <ScrollView showsVerticalScrollIndicator={false}>
            {/* Status Filter */}
            <View className="mb-4">
              <Text className="text-sm font-rubik-medium text-gray-700 mb-2">Status</Text>
                ...
                {[
                  ...
                  >
                    <Text className={`text-sm font-rubik ${
                      filters.status === option.key ? 'text-white' : 'text-gray-700'
                    }`}>
                      {option.label}
                    </Text>
                  </TouchableOpacity>
